...
def create_cms1500_healthcare_index(index_client, index_name, dimensions=3072):
    """
    Create or update an Azure AI Search index with fields specific to CMS-1500 healthcare claims
    ...
    fields = [
        ...
        # Patient information
        SearchableField(
            name="patientName", type=SearchFieldDataType.String, filterable=True
        ),
        SearchableField(
            name="patientAddress", type=SearchFieldDataType.String, filterable=True
        ),
        SimpleField(
            name="patientDOB",
            type=SearchFieldDataType.DateTimeOffset,
        ...
        SearchableField(
            name="providerNPI", type=SearchFieldDataType.String, filterable=True
        ),
        ...
        SearchableField(
            name="insuredID", type=SearchFieldDataType.String, filterable=True
        ),
    ...
    vector_search = VectorSearch(
        algorithms=[
            HnswAlgorithmConfiguration(
                ...
                parameters={"m": 10, "efConstruction": 400, "efSearch": 100},
            )
        ],
        profiles=[
            VectorSearchProfile(
    ...
    print(
        f"✅ CMS-1500 Healthcare Index '{result.name}' created with {dimensions} dimensions"
    )
    return result


def generate_embeddings(
    openai_client, text, model="text-embedding-ada-002", max_retries=3
):
    """
...
def prepare_cms1500_documents(excel_path):
    """
    Process an Excel file with CMS-1500 healthcare claims data and prepare structured documents
    for Azure AI Search with specialized fields for aggregation
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            doc = {"id": str(idx), "content": content, "isAggregationCandidate": True}
            ...
            if "Patient_Middle_Initial" in row and pd.notna(
                row["Patient_Middle_Initial"]
            ):
            ...
            if "Patient_Date_of_Birth" in row and pd.notna(
            ...
            if "Billing_Provider_Name" in row and pd.notna(
            ...
            if "Insurance_Company_Name" in row and pd.notna(
                row["Insurance_Company_Name"]
            ):
                doc["insuranceCompany"] = str(row["Insurance_Company_Name"])

            if "Insurance_Plan_or_Program_Name" in row and pd.notna(
                row["Insurance_Plan_or_Program_Name"]
            ):
        ...
        print(
            f"✅ Prepared {len(documents)} structured CMS-1500 healthcare claim documents for indexing"
        )
        return documents
...
def upload_documents_with_tracking(
    search_client,
    documents,
    openai_client,
    batch_size=25,
    ...
    """
    Upload documents to Azure Search in batches with progress tracking

    Args:
        search_client: Azure Search client
        documents: List of documents to upload
        openai_client: Azure OpenAI client for generating embeddings
        batch_size: Size of each upload batch
    ...
    batch_progress = tqdm(
        range(num_batches), desc="Uploading document batches", unit="batch"
    )
    ...
    for batch_idx in batch_progress:
        ...
        embedding_progress = tqdm(
            current_batch,
            desc=f"Generating embeddings for batch {batch_idx+1}/{num_batches}",
            leave=False,
            unit="doc",
        )

        for doc in embedding_progress:
            doc["vector"] = generate_embeddings(
                openai_client, doc["content"], embedding_model
            )

            for key, value in list(doc.items()):
                if key != "vector" and isinstance(value, list):
                    print(
                        f"Warning: Field {key} has array value. Converting to string."
                    )
        ...
        try:
            ...
            if batch_success < len(current_batch):
                print(
                    f"⚠️ {len(current_batch) - batch_success} documents failed in batch {batch_idx+1}"
                )
                for i, result in enumerate(results):
                    if not result.succeeded:
                        print(
                            f"  - Doc ID {current_batch[i]['id']} failed: {result.error_message}"
                        )
...
def index_cms1500_claims(
    ...
    """
    Complete workflow to index CMS-1500 healthcare claims data from Excel to Azure AI Search

    Args:
        excel_path: Path to the Excel file with claims data
        ...
        skip_index_creation: Skip index creation (just upload)
    ...
    print(
        f"🚀 Starting CMS-1500 healthcare claims indexing process at {datetime.now().strftime('%H:%M:%S')}"
    )
    ...
    print(
        f"🔄 Uploading {len(documents)} CMS-1500 healthcare claim documents to '{index_name}' (batch size: {batch_size})"
    )
    ...
    try:
        ...
        amounts = [
            ...
            if "claimAmount" in doc and doc["claimAmount"] is not None
        ]
        if amounts:
            avg_amount = sum(amounts) / len(amounts)
            print(
                f"   - Sample calculation: Average claim amount = ${avg_amount:.2f} based on {len(amounts)} claims"
            )
...
if __name__ == "__main__":
    ...
    if not all(
        ...
        print(
            "- EXCEL_FILE_PATH (optional, defaults to '50rowsrealisticdata_CLEANED.xlsx')"
        )
        exit(1)
